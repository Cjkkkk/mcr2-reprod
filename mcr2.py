...
class MaximalCodingRateReduction(torch.nn.Module):
    ...
    def compute_compress_loss_empirical(self, W, Pi):
        ...
        for j in range(k):
            ...
            # if math.isnan(log_det):
            #     print(torch.det(I + scalar * W.matmul(Pi[j]).matmul(W.T)), torch.det(scalar * W.matmul(Pi[j]).matmul(W.T)), log_det, scalar)
            #     print(Pi[j])
    ...
    def forward(self, X, Y, num_classes=None):
        ...
        W = X.T
        Y = Y.cpu()
        Pi = utils.label_to_membership(Y.numpy(), num_classes)
        ...
        # print(discrimn_loss_empi, compress_loss_empi)
